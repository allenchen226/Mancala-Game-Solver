...
from mancala_game import MancalaGameManager, AiPlayerInterface, Player, InvalidMoveError, AiTimeoutError, end_game, \
    get_possible_moves
...
class MancalaGui(object):
    ...
    def __init__(self, game_manager, player1, player2):
        ...
        self.root = root
        self.canvas = Canvas(root, height=self.cell_size * (self.height + 1) + self.offset,
                             width=self.cell_size * (self.width + 2) + self.offset)
    ...
    def draw_pits(self):
        ...
        # pits for players
        self.canvas.create_oval(self.offset, self.offset, self.cell_size + self.offset,
                                2 * self.cell_size + self.offset, fill="white")
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hl:d:a:b:t:c",
                                   ["limit=", "dimension=", "agent1=", "agent2=", "algorithm=", "caching="])
