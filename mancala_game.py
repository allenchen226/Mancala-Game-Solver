...
class MCTS:  # Use this to represent an MCTS tree
    ...
    def __str__(self):
        ...
        return "rewards: {}, counts: {}, states visited {}".format(sum(self.rewards.values()),
                                                                   sum(self.counts.values()), string)
...
class AiPlayerInterface(Player):
    ...
    def __init__(self, filename, color, limit, algorithm=2, caching=False):
        ...
        self.process.stdin.write(
            (str(color) + "," + str(limit) + "," + str(caching) + "," + str(algorithm) + "\n").encode("ASCII"))
...
def play_move(board, player, j):
    ...
    while (stone_count > 0):  # Deposit stones around the board
        ...
        if (ind > (len(board.pockets[side]) - 1) or ind < 0):
            ...
            if (ind > (len(board.pockets[
                               side]) - 1)):  # If we are at the end of the board, deposit stone in a mancala before we continue
                if (player == 1):
                    stone_count -= 1
